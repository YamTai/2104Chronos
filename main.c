...
//		display_chars(LCD_SEG_L1_3_0, "MODE", SEG_ON);
	...
	if (P2IFG == BUTTON_UP_PIN){
		display_chars(LCD_SEG_L1_3_0, "  ON", SEG_ON);
		buzz(1, 0x01FF);
		accel_start();

	}else
	if (P2IFG == BUTTON_DOWN_PIN){
		display_chars(LCD_SEG_L1_3_0, " OFF", SEG_ON);
		buzz(1, 0x01FF);
		accel_stop();
	}else
...
//	}else
	...
	P2IFG &= ~0x80;	//	have to clear P2.7 (buzzer), for some reason...
...
__interrupt void TIMER_A3_CCR1_CCR2_ISR(void){
	...
	if ((TA1CCTL2 & CCIFG) == 1){	//	P2.3 interrupt (CCI2A)
		...
		TA0CCR0 = rand + 0x1000;	//	set TA0CCR0 to 'random' value + offset of about .5 seconds(ACLK/8)
